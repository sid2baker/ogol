defmodule Ogol.Machine.Studio.Source do
  @moduledoc false

  alias Ogol.Machine.Studio.Artifact
  alias Ogol.Machine.Studio.Diagnostic
  ...
  @spec load_file(Path.t()) :: {:ok, Artifact.t()} | {:error, Artifact.t()}
  ...
  @spec load_source(String.t(), keyword()) ::
          {:ok, Artifact.t()} | {:error, Artifact.t()}
  ...
  @spec load_model_file(Path.t()) :: {:ok, Model.t()} | {:error, Artifact.t()}
  def load_model_file(path) do
    with {:ok, artifact} <- load_file(path),
         {:ok, model} <- Lowering.lower_artifact(artifact) do
      {:ok, model}
    else
      {:error, %Artifact{} = artifact} -> {:error, artifact}
      {:ok, %Artifact{} = artifact} -> {:error, artifact}
    end
  end
  ...
  defp finalize(%Artifact{} = artifact) do
    compatibility =
      cond do
