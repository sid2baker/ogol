defmodule Ogol.Machine.Studio.Printer do
  @moduledoc false
  ...
  @spec print(Model.t()) :: String.t()
  def print(%Model{} = model) do
    model
    |> to_quoted()
  ...
  @spec to_quoted(Model.t()) :: Macro.t()
  def to_quoted(%Model{} = model) do
    {:defmodule, [],
     [
  ...
  defp canonical_module(%Model{metadata: %{name: name}}) when is_atom(name) do
    Module.concat([Ogol, Authored, Macro.camelize(Atom.to_string(name))])
  end

  defp canonical_module(%Model{module: module}) when is_atom(module), do: module
