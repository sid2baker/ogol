defmodule Ogol.Session.HeldRunFaultClassificationScenarioTest do
  ...
  test "workspace drift classifies held runs as external-runtime operator-ack faults" do
    ...
    assert_eventually(
      fn ->
        ...
        assert run.fault_recoverability == :operator_ack_required
  ...
  defp drift_workspace!(original_source) when is_binary(original_source) do
    updated_source =
      String.replace(
        ...
        ~s|meaning("Commissioning cycle with held-fault classification")|,
        global: false
      )
